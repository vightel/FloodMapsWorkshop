...
function get_maxq_legend(req) {
	...
	html += "<div id='maxq_map_legend' class='maxq_map-info'>"
...
module.exports = {
	maxq: function(req, res) {
		var style 	= get_maxq_style(req);
		var html  	= get_maxq_legend(req);
		var credits = get_maxq_credits(req);
		res.render("mapinfo/maxq", { style: style, html: html, credits: credits })
	},
	maxq_style: function(req, res) {
		var json = get_maxq_style(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'application/json');		
		res.send(json)
	},
	maxq_legend: function(req, res) {
		var html = get_maxq_legend(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'text/html');		
		res.send(html)
	},
	maxq_credits: function(req, res) {
		var str = get_maxq_credits(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'application/json');		
		res.send(str)
	}
