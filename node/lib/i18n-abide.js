...
// Pat Cappelaere - Modified to support transifix and the Mapbox iD way
...
exports.abide = function(options) {
  ...
  function messages_file_path(locale) {
    ...
    if( options.translation_type === 'transiflex') {
        var filename = locale + '.json';
        return path.resolve(path.join(__dirname, "..", options.translation_directory, filename));
    }
  ...
  function parse_messages_file(locale) {
    ...
    if (options.translation_type === 'transiflex') {
        ...
        var json = JSON.parse(rawMessages);
        return json 
    }
    ...
    // PO->json file
      ...
      var rawMessages = fs.readFileSync(filepath).toString();
      var json = JSON.parse(rawMessages).messages;
      return json
    }
  }
