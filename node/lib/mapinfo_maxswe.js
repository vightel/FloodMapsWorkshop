function get_maxswe_credits(req) {
	var json = {
...
function get_maxswe_legend(req) {
	...
	html += "<div id='maxswe_map_legend' class='maxswe_map-info'>"
...
module.exports = {
	maxswe: function(req, res) {
		var style 	= get_maxswe_style(req);
		var html  	= get_maxswe_legend(req);
		var credits = get_maxswe_credits(req);
		res.render("mapinfo/maxswe", { style: style, html: html, credits: credits })
	},
	maxswe_style: function(req, res) {
		var json = get_maxswe_style(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'application/json');		
		res.send(json)
	},
	maxswe_legend: function(req, res) {
		var html = get_maxswe_legend(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'text/html');		
		res.send(html)
	},
	maxswe_credits: function(req, res) {
		var str = get_maxswe_credits(req)
	    res.header("Access-Control-Allow-Origin", "*");
		res.set('Content-Type', 'application/json');		
		res.send(str)
	}
