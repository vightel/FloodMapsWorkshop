var util 		= require('util'),
	...
	function QueryByID(req, user, year, doy, credentials, cb ) {
		...
		var duration		= 60 * 30
		var id				= "trmm_"+ year.toString() + doy
		...
		var key =  "trmm" + "/" + date.year() + "/" + doy + "/" + "trmm_24."+date.year()+month+day+".tif"
		...
		app.s3.headObject(params, function(err, data) {
			...
			var browse_img_url		= s3host+"trmm_24."+year+month+day+"_thn.jpg"
			var topojson_url		= s3host+"trmm_24."+year+month+day+".topojson"
			var topojson_file		= s3host+"trmm_24."+year+month+day+".topojson.gz"
			...
			var source 		= req.gettext("sources.trmm")
			var sensor 		= req.gettext("sensors.trmm")
