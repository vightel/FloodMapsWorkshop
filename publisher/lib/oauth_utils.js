/**
   Utils for oauth server.
	https://github.com/KirillTemnov/oauth-server/blob/master/lib/util.js
 */
var URL 	= require('url'),
	sys 	= require('sys'),
...
var keys = [
	{
		"key": "5A043yRSEugj4DJ5TljuapfnrflWDte8jTOcWLlT",
		"secret": "aB3jKq1TRsCOUrfOIZ6oQMEDmv2ptV76PA54NGLL"
	},
	{
		"key": "zwQZFivccHkLs3a8Rq5CoS412fE5aPCXDw9DZj7R",
		"secret": "aMnOOCwExO2XYtRVWJ1bI9QOdqh1cay2UgpbhA6p"
	}
];
	
function lookupConsumer(oauth_consumer_key, cb) {
	var consumer = _.find( keys, function(k) {
		return ( k.key === oauth_consumer_key )
	})
...
exports.checkRequest = function (req, store, fn) {
    ...
    if (authorization.indexOf('OAuth') >= 0 ) {
  	  var auth = authorization.replace("OAuth ", "").split(', ');
		//console.log("Oauth headers", auth)
		
        // parsedParams
        var pp = sortParams(auth.map(function (p) {
            var arr= p.split('=')
		...
		//console.log("checkRequest params", params)
		
		lookupConsumer(params.oauth_consumer_key, function (consumer) {
            if (consumer) {			
