...
def get_daily_trmm_files(trmm_gis_files, mydir, year, month):
	filepath = gis_path+ "%d%02d" % (year, month)
...
def process(trmm_dir, gis_file_day, region, s3_bucket, s3_folder, ymd ):
	# subset the file for that region
	bbox		= region['bbox']
	gis_file	= os.path.join(BASE_DIR, trmm_dir, gis_file_day)
	subset_file	= os.path.join(BASE_DIR, trmm_dir, "trmm_24.%s.tif" % ymd)
	...
	geojsonDir	= os.path.join(trmm_dir,"geojson")
	if not os.path.exists(geojsonDir):            
		os.makedirs(geojsonDir)

	levelsDir	= os.path.join(trmm_dir,"levels")
	if not os.path.exists(levelsDir):            
		os.makedirs(levelsDir)

	merge_filename 		= os.path.join(geojsonDir, "trmm_24.%s.geojson" % ymd)
	topojson_filename 	= os.path.join(geojsonDir, "..", "trmm_24.%s.topojson" % ymd)
	browse_filename 	= os.path.join(geojsonDir, "..", "trmm_24.%s_browse.tif" % ymd)
	subset_filename 	= os.path.join(geojsonDir, "..", "trmm_24.%s_small_browse.tif" % ymd)
	osm_bg_image		= os.path.join(geojsonDir, "..", "osm_bg.png")
	sw_osm_image		= os.path.join(geojsonDir, "..", "trmm_24.%s_thn.jpg" % ymd)
...
# Main
#
# python trmm_24.py --date 2015-03-27 -v -f
...
if __name__ == '__main__':
	...
	trmm_dir	= os.path.join(BASE_DIR,str(year),doy)
	...
	gis_file_day		= "3B42RT.%04d%02d%02d21.7.1day.tif"%(year, month, day)
	gis_file_day_tfw 	= "3B42RT.%04d%02d%02d21.7.1day.tfw"%(year, month, day)
	
	if force or not os.path.exists(os.path.join(trmm_dir,gis_file_day)):
		get_daily_trmm_files([gis_file_day, gis_file_day_tfw], trmm_dir, year, month)
	
	process(trmm_dir, gis_file_day, region, s3_bucket, s3_folder, ymd)
