...
class Landsat8:
	def __init__( self, outpath, scene, rgb ):	
		...
		self.output_file		= os.path.join(outpath, rgb + ".PS_BROVEY.TIF")
	...
	def process(self):
		if verbose:
			print "Opening Pan File", self.pan_file
		...
		if verbose:
			print "Opening RGB File", self.rgb_file
		...
		sum_data	= red_data + green_data + blue_data
		mask		= (sum_data == 0)
		sum_data[mask]	= 1
		
		mult		= pan_data / sum_data
		mult[mask]  = 0
